...
D_MODEL = 128                       # 输入、输出词向量维数
# D_FF = 1024                         # feed forward全连接层维数
D_FF = 512                          # feed forward全连接层维数
DROPOUT = 0.1                       # dropout比例
MAX_LENGTH = 60                     # 语句最大长度

TRAIN_FILE = 'nmt/cn-en/train_mini.txt'  # 训练集
DEV_FILE = "nmt/cn-en/dev_mini.txt"  # 验证集
SAVE_FILE = 'save/model_mini.pt'         # 模型保存路径
...
class PrepareData:
    def __init__(self, train_file, dev_file):
        ...
        self.train_data = self.split_batch(self.train_cn, self.train_en, BATCH_SIZE)
        self.dev_data = self.split_batch(self.dev_cn, self.dev_en, BATCH_SIZE)
...
def evaluate(data, model):
    ...
    with torch.no_grad():
        ...
        for i in range(len(data.dev_en)):
            ...
            # 打印待翻译的中文语句
            en_sent = " ".join([data.cn_index_dict[w] for w in data.dev_cn[i]])
            print("\n" + en_sent)

            # 打印对应的英文语句答案
            cn_sent = " ".join([data.en_index_dict[w] for w in data.dev_en[i]])
            print("".join(cn_sent))
